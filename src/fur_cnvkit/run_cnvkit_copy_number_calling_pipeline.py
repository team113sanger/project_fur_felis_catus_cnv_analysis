...
def setup_logging():
    """
    Set up logging configuration for debugging and traceability.
    """
    logging.basicConfig(
...
def perform_post_processing(
    ...
    """
    logging.info(f"Post-processing sample {sample_id} ...")
    ...
    logging.debug(f"Sample {sample_id} - Ratio file: {ratio_file}")
    logging.debug(f"Sample {sample_id} - Call segment file: {call_segment_file}")
    ...
    logging.debug(f"Sample {sample_id} - Log2 shift value: {log2_shift_value}")
    ...
    logging.debug(f"Log2 shift record appended for sample {sample_id}")
    ...
    logging.debug(f"Sample {sample_id} - Diagram plot: {diagram_plot}")
    logging.debug(f"Sample {sample_id} - Scatter plot: {scatter_plot}")

    logging.info(f"Sample {sample_id} post-processing complete.")
...
def process_sample(
    ...
    logging.debug(f"Sample {sample_id} - CNVkit output files: {sample_files}")
    ...
    for file in sample_files:
        ...
        logging.debug(f"Filtered unplaced contigs in file: {file}")
...
def process_sex_group(
    ...
    """
    logging.info(f"Processing {sex} samples ...")
    # Choose the appropriate reference based on sex.
    copy_number_reference_file = male_ref if sex == "male" else female_ref
    logging.debug(f"{sex.capitalize()} reference file: {copy_number_reference_file}")
    ...
    logging.debug(f"Batch output directory for {sex} samples: {batch_output_dir}")
    ...
    logging.info(f"Found {len(sex_sample_ids)} {sex} samples.")
...
def generate_genemetrics_study_summary_csv(
    ...
    logging.debug(f"Output CSV path: {output_csv_file_path}")

    # Load baitset genes.
    with open(baitset_genes_file, "r") as f:
        baitset_genes_list = [line.strip() for line in f.readlines()]
    logging.debug(f"Baitset genes (first 5): {baitset_genes_list[:5]}")
...
def process_study(
    ...
    logging.debug(f"Study output directory: {study_outdir}")
    ...
    logging.info(f"Study {study_id} - Found {len(study_tumour_bams)} tumour BAM files.")

    # Split the tumour BAMs by sex.
    tumour_bam_sex_dict = split_file_list_by_sample_sex(
        study_tumour_bams, sample_metadata_xlsx
    )
    logging.debug(f"Tumour BAMs split by sex: {tumour_bam_sex_dict}")
    ...
    logging.info(f"Log2 shift CSV saved at {log2_shift_csv_path}")
    ...
    logging.info(f"Study {study_id} processing complete.")
...
def main():
    ...
    logging.debug(f"Male reference: {male_ref}")
    logging.debug(f"Female reference: {female_ref}")
    ...
    logging.info("Starting CNVkit copy number calling pipeline ...")
    ...
    logging.debug(f"Sample metadata file: {sample_metadata_xlsx}")
    ...
    logging.debug(f"Mapped study IDs to sample IDs: {study_ids_to_sample_ids}")
    ...
    logging.info("CNVkit pipeline execution complete.")
